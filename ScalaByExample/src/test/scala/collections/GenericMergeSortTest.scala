...
class GenericMergeSortTest extends FunSuite {
  ...
  test("Test with multi item List") {
    val sList = GenericMergeSort.msort(List(1, -4, 8, 2, 3, 0))(Ordering.Int)
    assert(sList.head < 0, "Now first element is negative")
  }

  test("Test with boolean items List") {
    val sList = GenericMergeSort.msort(List(true, false, false, true))(Ordering.Boolean)
    //assert(sList.head, "Now first element is negative")
    println(sList)
  }
  test("Test with string items List") {
    val sList = GenericMergeSort.msort(List("xxx", "aaa", "fff", "ddd", "zzz"))(Ordering.String)
    println(sList)
  }
}